/**
 * Agent Detail – Fetches agent config (from list) and events via GET /api/v1/agents/{id}/events.
 * Renders overview, full configuration, and events with image/video evidence.
...
(function () {
  ...
  // ---------------------------------------------------------------------------
  // Status helpers (mirrors agents-board.js)
  // ---------------------------------------------------------------------------
  ...
  function loadEventImagesWithAuth(container) {
    if (!container || !window.visionAPI || typeof window.visionAPI.fetchEventImageObjectUrl !== 'function') return;
    container.querySelectorAll('.agent-detail-event-card[data-event-id]').forEach(function (card) {
      ...
      if (!link || !img) return;
      window.visionAPI.fetchEventImageObjectUrl(eventId)
        .then(function (url) {
          link.href = url;
    ...
    if (backLink) {
      ...
      backLink.addEventListener('click', function (e) {
        if (window.visionaiSpa && typeof window.visionaiSpa.navigate === 'function') {
          e.preventDefault();
          window.visionaiSpa.navigate(agentsBoardHref);
        }
      });
    }
    ...
    if (stopBtn) {
      stopBtn.addEventListener('click', function () {
        ...
        apiRequest('/api/v1/agents/stop/' + encodeURIComponent(agentId), { method: 'POST' })
          .then(function () {
            if (window.VisionToast && window.VisionToast.success) {
              window.VisionToast.success('Agent stopped.');
            }
            setTimeout(boot, 800);
          })
          .catch(function (err) {
    ...
    if (pauseBtn) {
      pauseBtn.addEventListener('click', function () {
        ...
        apiRequest('/api/v1/agents/pause/' + encodeURIComponent(agentId), { method: 'POST' })
          .then(function () {
            if (window.VisionToast && window.VisionToast.success) {
              window.VisionToast.success('Agent paused.');
            }
            setTimeout(boot, 800);
          })
          .catch(function (err) {
    ...
    if (resumeBtn) {
      resumeBtn.addEventListener('click', function () {
        ...
        apiRequest('/api/v1/agents/resume/' + encodeURIComponent(agentId), { method: 'POST' })
          .then(function () {
            if (window.VisionToast && window.VisionToast.success) {
              window.VisionToast.success('Agent resumed.');
            }
            setTimeout(boot, 800);
          })
          .catch(function (err) {
